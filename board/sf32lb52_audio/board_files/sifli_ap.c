...
int sf32lb52_lchspi_ulp_bringup(void)
{
   ...
   * through the Class-D PA (enable on PA10).
   *
   * audio_register() takes the *name* only: with CONFIG_AUDIO_DEV_ROOT
   * it builds "/dev/" + name (nuttx/audio/audio.c:1808), so passing the
   * full "/dev/audio0" here would register "/dev//dev/audio0" instead. */
  ...
  if (audio == NULL)
    ...
    {
      ret = audio_register("audio0", audio);
      if (ret < 0)
        {
