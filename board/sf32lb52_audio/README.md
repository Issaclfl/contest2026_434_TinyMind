...
### 7.11 离线控制闭环：本地命令模型 → 板子播报 + 上屏（2026-09-20）

**先说清用哪个模型**。S3 上跑着两个本地模型，性质完全不同：

- `stories260K`（0.26M）：离线**聊天兜底**，官方文档自己写着"只会写通顺的英文小故事、
  不会听指令"，不能拿它做控制。
- **`cmd`：自训的命令模型**（`esp32s3_common/tools/train_cmd_model.py`），`/talk` 页面
  用的就是它，输出是**动作 JSON**——这才是"离线控制"的载体。上游用它，我们也用它。

**对接链路**（网关侧上一轮已在真机验证，板侧今天补齐）：

| 环节 | 做法 |
|---|---|
| 板子配置 | `set_llm http://10.0.0.1/v1 <占位key> cmd` |
| 网关侧 | 请求里 `model == "cmd"`（`net_cloud.c` 的 `CMD_MODEL_NAME`）→ 本地命令模型解析 → **在本机执行动作** → 按 OpenAI 格式回包，动作 JSON 放在 `choices[0].message.content` |
| 板子侧 | 新增 `src/voice/action_speak.c`：识别回复是动作 JSON → 翻成中文上屏 + 播放对应离线片段 |

**为什么这条才算"断网可用"**：动作 JSON 是机器语言，站在板子旁边的人要听的是"红灯已打开"。
动作 JSON → 中文 → 音频全部在本地完成，既不查云也不调 TTS。映射表：

| 动作 | 屏上 / 播报 | 片段 |
|---|---|---|
| `led` + `red` / `blue` / 其它颜色 / `off` | 红灯已打开 / 蓝灯已打开 / 客厅灯已打开 / 客厅灯已关闭 | red_on / blue_on / light_on / light_off |
| `led` + `blink`(+`times`) | 灯已闪烁N次 | ok |
| `led` + `breath` | 灯已设为呼吸效果 | ok |
| `status` / `temperature` | 状态正常 / 芯片温度N度 | ok |
| `wifi_scan` / `ping` | 正在扫描无线网络 / 网络连通性检查完成 | ok |
| `weather` | **天气需要联网，当前处于离线模式**（如实说，不编） | offline |
