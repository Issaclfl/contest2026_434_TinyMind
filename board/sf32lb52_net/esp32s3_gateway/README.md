...
| **云-端自适应路由** | **路由逻辑已实测**（同日，从 WiFi 侧直接对端点发请求）：`auto` 无 key → 本地作答并计数 `no-key`、`local` → 计数 `direct local`、`GET /` 仪表盘与 `GET /status` 正常。云端腿（代理转发、断网降级）**待 key 输入后实测**——key 属凭据，只进 gitignore 的 `sdkconfig`，由使用者在自己的终端经 `tools/set_cloud.py` 输入 |
...
- 板侧用法不变：`set_llm http://10.0.0.1/v1 auto local`（末位参数只是占位）。
  演示断网降级：正常问一轮 → 关热点 → 再问（本地接住）→ 开热点 → 再问（云端回来）。
