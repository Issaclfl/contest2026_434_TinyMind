...
**端到端已在真机验证（2026-09-18，三根杜邦线）**：板子 Agent 的请求经 PPP
进入 S3，S3 上的本地 llama2.c 模型生成回答并原路返回——`ask` 一句、12.4 s
得到 576 字符回答，全程无 PC、无互联网；S3 重启后板子 6.9 s 自动重拨。此前的
台架实测（对 PC：LCP/IPCP 协商、ping 20/20、满 MTU 通路）与日志原文见
`../../docs/ESP32-S3网关台架实测证据.md`。唯一未验的是 NAPT 转发
（板子经 S3 上公网），只差一次 Wi-Fi 凭据的填入。
...
**路线 1 与路线 2 的软件链路都已在真机上端到端跑通**；路线 3 也已在
2026-09-18 台架实测中分三步真机验证完毕（S3↔PC 互通 → 板子↔S3 全链路 →
板子经 S3 上公网），三步的日志原文都在 `../../docs/ESP32-S3网关台架实测证据.md`。
...
| **路线 3：板子 ↔ S3 全链路（真机）** | 三根杜邦线（GPIO17→PA20、GPIO18←PA27、GND）；S3 同时跑本地模型与 OpenAI 兼容端点 | **通**：板子 `ask` 一句 → Agent 请求（15.5 KB）经 PPP 进 S3 → 本地模型生成 → 12.4 s 返回 576 字符并在板子显示；S3 重启后板子 6.9 s 自动重拨 |
| **路线 3：板子经 S3 上公网（NAPT，真机）** | S3 连 2.4 GHz 热点（`uplink ready: 10.138.138.122`），板子经三根线拨入 | **通**：板子 `ping 223.5.5.5` **4/4、0% 丢包、平均 80 ms**；期间本地模型端点持续在线。逼出并修复 AMPDU ROM 打印挤爆 PPP UART 接收环的真实故障（提交 ac6f6fb） |
