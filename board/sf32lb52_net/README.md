...
**状态要说清楚：软件已完成、`idf.py build` 通过、关键符号已核对，但没有上板，
互通性未验证。** 这一条与路线 1/2 的性质不同，不要混为一谈。
...
**路线 1 与路线 2 的软件链路都已在真机上端到端跑通**；路线 3 只做到编译验证，
所以单独列在表末，不计入"已验证"。
...
| 路线 3：ESP32-S3 网关 | `idf.py build` + 核对生成的 `sdkconfig.h` | **软件完成，未上板**：与板子的互通、NAPT 转发、断线重连都还没验 |
