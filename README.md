...
文本对话已经在真机上跑通；语音交互（唤醒词统一用「你好，openvela」）依赖 Vela 媒体框架，属下一步工作。**
...
**接线速查：板子 ↔ ESP32-S3 网关（三根杜邦线，交叉接）**

| ESP32-S3-DevKitC-1 | 接到 | SF32LB52 板 | 板侧信号名 |
|---|---|---|---|
| **GPIO17**（UART1 TX） | → | **PA20** | `USART2_RXD`（板子收） |
| **GPIO18**（UART1 RX） | ← | **PA27** | `USART2_TXD`（板子发） |
| **GND** | ↔ | **GND** | 必须共地 |

- **不要互接电源**（3V3/5V），两块板各自供电；只接 TX/RX/GND 三根。
- S3 侧别碰 **GPIO19/20**（原生 USB）与 **GPIO43/44**（它的控制台），要接的是丝印 `IO17`/`IO18`。
