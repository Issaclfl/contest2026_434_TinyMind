...
### 8.5 一个新的间歇症状：答复算出来但不派发（未定位）

2026-09-20 复核时遇到一轮：

```
[trace:7c5450992746feb7] END status=ok iters=1 tools=0 llm_ms=5350 elapsed=5s backend=0
（然后就没有下一行了 —— 既没有 [agent] Dispatching response，也没有 [Agent]: 台灯已打开）
```

即：LLM 的答复已经拿到（`[llm] Response: 15 bytes text`，15 字节正好是 `台灯已打开`），
但**没有派发到控制台、屏幕与喇叭**。同一轮里连着的三次提问都没派发；
下一轮（同一台板子、同一条链路、什么都没重启）第一句就正常显示并播报了。

- 影响：演示时表现为"只有'思考中'，没有答复"。
- 规避：**原样再问一次**——Agent 命中缓存立即返回同一答案并派发，不必重启板子。
- 状态：**未定位**。它与 `ask` 后的间歇崩溃（§11.5）可能是同一族的时序问题，但两者
  都没有抓到断言/回溯原文，不做因果推断。
