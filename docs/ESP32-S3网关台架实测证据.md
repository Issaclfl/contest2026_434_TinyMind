...
| 训练集 | 1611 条 | 1637 条（模板 + 蒸馏 197 + 手工 60） |

第一版的错法很规律：`关闭台灯`、`关闭客厅的灯` 被判成"开灯"。根因是蒸馏语料里
"关"族说法只有 `关了 / 关一下 / 关掉 / 关空调`，**没有"关闭"这个动词**。
补的手工语料单独放 `zh_hand.jsonl`，训练脚本加了 `--extra` 参数，
合并步骤因此可复现（原来是手工做的）。

第二轮补极值/百分比/抱怨式说法后，逐句探针（`tools/probe_clf.py`）：

```
把台灯调到最亮      -> brightness 100
台灯调到最暗       -> brightness 20
台灯开到最亮       -> brightness 100
台灯调到百分之五十    -> brightness 50
灯太暗了          -> brightness 80
灯太亮了          -> brightness 30
```
...
`pc_side/s3_cmd_probe.py`（新增）把 15 句口语直接打给 S3 的 `model:"cmd"`：
...
ok   台灯  关闭客厅的灯       -> 台灯已关闭
ok   台灯  关灯           -> 台灯已关闭
ok   台灯  台灯调亮一点       -> 台灯亮度已调到 80
ok   台灯  把台灯调到最亮      -> 台灯亮度已调到 100
ok   台灯  台灯暗一点        -> 台灯亮度已调到 30
ok   台灯  台灯调暖一点       -> 台灯色温已调到 2700K
ok   空调  打开空调         -> 空调已打开
ok   空调  把空调打开        -> 空调已打开
ok   空调  关闭空调         -> 空调已关闭
ok   空调  把空调关掉        -> 空调已关闭
...
第一次跑是 9/15：两条 `关闭…` 错、四条空调"没有回应"。

### 空调"没有回应"的真因：token 不匹配

设备表里 `ac` 行原来配的是另一个 token，而**官方模拟器的 token 是固定全零的公开常量**
（`miio/devtools/simulators/common.py`：`"token": 32 * "0"`）。token 对不上就解不开回包，
网关如实回"空调没有回应"——**不是分类器的问题**，分类器把四句都正确判成了 `ac on/off`。

两行都指向同一个官方模拟器实例即可。要如实说的边界：官方模拟器端口写死 54321，
一台主机只能起一个实例，所以演示里"台灯"和"空调"两个设备名**共用它**；
协议与代码路径和真机一致，差别只在设备表里那一行的标签。

### 已知缺陷：S3 的 httpd 会拒绝新连接

现象（S3 控制台刷屏，PC 侧连接被 RESET）：

```
tpd: httpd_server: error accepting new connection
```

- 出现时机：S3 跑一段时间之后（不是立刻）。
- 影响面：**PC → S3 的 HTTP/HTTPS 连不上**；板子经 PPP 的 HTTPS **不受影响**
  （上面 15/15 与板子四句批测都是在同一段时间里跑通的）。
- 处置：复位 S3 即恢复（`tools/s3_reset.py`，DTR 脉冲）。演示前复位一次即可。
- 未定位根因，按"已知缺陷"记录，不写成已解决。

### 板子控制台"失联"的救活办法（运维小记）

板子有若干种把自己搞哑的方式（`quit`/`restart`、以及控制台在异常状态下只剩回显）。
软件侧能救回来的办法（比重新烧整包快得多，约 10 秒）：

```bash
sftool.exe -c SF32LB52 -p COM5 -b 1000000 \
  --before default_reset --after soft_reset \
  write_flash <镜像的前 4KB>@0x12010000        # 或 read_flash <任意文件>@0x12010000:0x100
```

关键是 `--after soft_reset` 这一步把芯片从 ROM 下载态里带出来。
另外实测：**pyserial 开端口时不要动 DTR/RTS**（本仓库的 `pc_side/serial_tcp_bridge.py`
就是这么做的，已经跑通）；手工把 RTS 拉高/拉低反而会把芯片按在下载态，
表现出来就是"串口只有回显、没有任何命令输出"。
